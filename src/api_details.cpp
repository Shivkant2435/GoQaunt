...
// for handling JSON data in data
// header only lib : no aditional compilation or linking steps are required.
using json = nlohmann::json;

bool AUTH_SENT = false;

// Further more currencies can be added .
vector<string> SUPPORTED_CURRENCIES = {"BTC", "ETH", "SOL"};

string deribit_api::start(const string &input)
{
    ...
    map<string, function<string(string)>> action_map =
        {
            ...
            {"buy", deribit_api::buy},
            {"get_orderbook", deribit_api::get_orderbook},
...
string deribit_api::authorize(const string &input)
{
...
string deribit_api::buy(const string &input)
{
    string buy;
    string id;
    string instrument;
    ...
    // values are taken from the API documentation by the debrit API .
    ...
    if (!std::any_of(permitted_order_types.begin(), permitted_order_types.end(), [&](string val)
                     { return val == order_type; }))
    {
    ...
    vector<std::string> permitted_tif = {"good_til_cancelled", "good_til_day", "fill_or_kill", "immediate_or_cancel"};
    if (!std::any_of(permitted_tif.begin(), permitted_tif.end(), [&](std::string val)
                     { return val == frc; }))
    {
...
string deribit_api::get_orderbook(const std::string &input)
{
    std::istringstream is(input);

    int id;
    string cmd;
    string opt1; // for : /public/get_book_summary_by_currency

    string opt2; // for : /public/get_book_summary_by_instrument
    is >> id >> cmd >> opt1 >> opt2;
...
std::string deribit_api::sell(const std::string &input)
{
    ...
    if (!std::any_of(permitted_order_types.begin(), permitted_order_types.end(), [&](std::string val)
                     { return val == order_type; }))
    {
        utils::printerr("\nIncorrect syntax; couldn't place order\nOrder type can be only one of \nlimit\nstop_limit\ntake_limit\nmarket\nstop_market\ntake_market\nmarket_limit\ntrailing_stop\n");
        return "";
