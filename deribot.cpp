...
#include <chrono> // for measuring the performance metrics
...
public:
    ...
    void record_sent_message(std::string const &message)
    {
        ...
        send_time = std::chrono::high_resolution_clock::now(); // Capture the send timestamp
    }
    ...
    void on_message(websocketpp::connection_hdl hdl, client::message_ptr msg)
    {
        ...
        // TEMP FOR MEASURING THE PERFORMANCE
        auto receive_time = std::chrono::high_resolution_clock::now(); // Capture the receive timestamp
        auto latency = std::chrono::duration_cast<std::chrono::milliseconds>(receive_time - send_time).count();
        std::cout << "WebSocket message propagation delay: " << latency << " ms" << std::endl;
...
public:
    ...
    int connect(std::string const &uri)
    {
        ...
        // start the record time for this :
        metadata_ptr->m_start_time = std::chrono::high_resolution_clock::now();
...
int main()
{
    ...
    fmt::print(fg(fmt::color::green) | fmt::emphasis::bold,
               "--------------------------GOQaunt Assignment -(Trade on Debribit Test)--------------------------\n");
    ...
    while (!done)
    {
        ...
        else if (input.substr(0, 4) == "send")
        {
            ...
            endpoint.get_metadata(id)->cv.wait(lock, [&]
        ...
        else if (input.substr(0, 7) == "DERIBIT")
        {
            ...
            std::string msg = deribit_api::start(input);
            if (msg != "")
            {
                ...
                if (success >= 0)
                {
                    ...
                    auto loop_latency = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
                    std::cout << "Order Placement Latency" << loop_latency << " ms" << std::endl;
                    std::cout << "Market Data Processing Latency:" << loop_latency << " ms" << std::endl;
